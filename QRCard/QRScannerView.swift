...
struct QRScannerView: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                // Camera view
                QRScannerPreviewView(scanner: scanner)
                    .ignoresSafeArea()
    ...
    private func handleScannedCode(_ code: String) {
        print("📱 QRScannerView: Scanned code: \(code)")
    ...
    private func parseQRCode(_ code: String) -> ContactCreateData? {
        ...
        if code.hasPrefix("http://") || code.hasPrefix("https://") {
            // For now, we'll just create a basic contact with the URL
            return ContactCreateData(
                ...
                country: nil,
                website: code,
                notes: "Scanned from QR code: \(code)",
                commentsFromLead: nil,
                birthdate: nil,
                photoUrl: nil,
                source: "qr_scan",
                sourceMetadata: "{\"qr_data\":\"\(code)\"}"
            )
        }
...
#Preview {
    QRScannerView(viewModel: ContactsViewModel())
}