<?php
/**
 * Generate HTML Name Tags for Word/Google Docs - QR Surround Variant
...
if (strlen($topBannerText) > 100) {
    http_response_code(400);
    exit('Top banner text is too long');
}

if (strlen($bottomBannerText) > 100) {
    http_response_code(400);
    exit('Bottom banner text is too long');
}

if (!preg_match('/^#[0-9A-Fa-f]{6}$/', $topBannerColor)) {
    http_response_code(400);
    exit('Invalid top banner color format');
}

if (!preg_match('/^#[0-9A-Fa-f]{6}$/', $bottomBannerColor)) {
    http_response_code(400);
    exit('Invalid bottom banner color format');
}
...
try {
    ...
    // Generate HTML for QR surround name tags
    $generator = new NameTagGenerator();
    $html = $generator->generateQRSurroundNameTagHTML($cardId, $preferences);
    ...
    // Log image creation
    $imageFilename = "name-tag-qr-surround-html-{$cardId}.html";
    ...
    error_log('QR Surround name tag HTML generation error: ' . $e->getMessage());
...
<head>
    ...
    <title>Hello My Name Is - Name Tag HTML - ShareMyCard</title>
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h1>👋 Hello My Name Is - Name Tag HTML Generator</h1>
