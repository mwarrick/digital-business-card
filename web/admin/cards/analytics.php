...
// Get all cards for dropdown (admins can view any card's analytics)
$db = Database::getInstance();
$cards = $db->query(
    "SELECT id, first_name, last_name FROM business_cards WHERE is_active = 1 ORDER BY created_at DESC"
);
...
// Get card details (admins can view any card's analytics)
$selectedCard = null;
if ($selectedCardId) {
    $selectedCard = $db->querySingle(
        "SELECT id, first_name, last_name, job_title, company_name, user_id FROM business_cards WHERE id = ? AND is_active = 1",
        [$selectedCardId]
    );
}
...
<head>
    ...
    <title>Card Analytics - ShareMyCard Admin</title>
...
<body>
    ...
    <?php if (!$selectedCardId || !$selectedCard): ?>
        <div class="info-box">
            ...
            <?php if (empty($cards)): ?>
                You don't have any business cards yet. <a href="/admin/cards/create.php">Create one now</a>!
            <?php else: ?>
                Please select a card from your <a href="/admin/my-cards.php">My Cards</a> page.
            <?php endif; ?>
        </div>
    <?php else: ?>
...
<script>
    ...
    function exportToCSV() {
        ...
        let csv = 'Date,Total Views,Unique Views,Clicks,Downloads,QR Scans\n';
        analyticsData.time_series.forEach(row => {
            csv += `${row.date},${row.views},${row.unique_views},${row.clicks},${row.downloads},${row.scans || 0}\n`;
        });
