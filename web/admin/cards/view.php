...
<body>
    ...
    <div class="main-container">
        ...
        <div class="card-actions" style="margin-bottom: 30px; justify-content: flex-start;">
            <a href="/admin/cards/edit.php?id=<?php echo urlencode($cardId); ?>" class="btn btn-primary">
                ✏️ Edit Card
            </a>
            <button onclick="duplicateCard('<?php echo $cardId; ?>')" class="btn btn-secondary" style="background: #17a2b8; color: white; border: none;">
                📋 Duplicate Card
            </button>
            <a href="/admin/cards/analytics.php?card_id=<?php echo urlencode($cardId); ?>" class="btn btn-secondary" style="background: #667eea; color: white; border: none;">
                📊 View Analytics
            </a>
            <a href="/admin/cards/qr.php?id=<?php echo urlencode($cardId); ?>" class="btn btn-secondary">
                📱 Generate QR Code
            </a>
                ...
                <div class="image-display profile">
                    <?php if (!empty($card['profile_photo_path'])): ?>
                        <img src="/api/media/view?filename=<?php echo urlencode($card['profile_photo_path']); ?>" 
                             alt="Profile Photo"
                             onerror="this.parentElement.innerHTML='<div class=\'image-placeholder\'><div class=\'icon\'>👤</div><div class=\'text\'>Failed to load photo</div></div>'">
                    <?php else: ?>
                ...
                <div class="image-display cover">
                    <?php if (!empty($card['cover_graphic'])): ?>
                        <img src="/api/media/view?filename=<?php echo urlencode($card['cover_graphic']); ?>" 
                             alt="Cover Graphic"
                             onerror="this.parentElement.innerHTML='<div class=\'image-placeholder\'><div class=\'icon\'>🎨</div><div class=\'text\'>Failed to load cover</div></div>'">
                    <?php else: ?>
                        <div class="image-placeholder">
                            <div class="icon">🎨</div>
        ...
        <div class="card-preview" style="margin-top: 30px;">
            ...
            <div style="text-align: center;">
                <a href="/admin/cards/email-signature.php?card_id=<?php echo urlencode($card['id']); ?>" 
    ...
    <div id="shareModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); z-index: 9999; align-items: center; justify-content: center;">
        <div style="background: white; padding: 40px; border-radius: 20px; max-width: 500px; width: 90%; max-height: 90vh; overflow-y: auto;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
                <h2 style="margin: 0; font-size: 24px;">Share Card</h2>
    ...
    <script>
        ...
        function duplicateCard(cardId) {
            if (confirm('Are you sure you want to duplicate this card? This will create a complete copy with all contact information.')) {
                ...
                fetch('/admin/api/duplicate-card.php', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = '/admin/cards/edit.php?id=' + data.new_card_id;
                    } else {
